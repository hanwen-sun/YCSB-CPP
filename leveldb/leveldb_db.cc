...
void LeveldbDB::Init() {
  ...
  if (format == "single") {
    ...
    method_report_ = &LeveldbDB::GenSingleReport;
    method_property_ = &LeveldbDB::GetLevelDBProperty;
  } else if (format == "row") {
    format_ = kRowMajor;
...
void LeveldbDB::GenSingleReport() {
  //fprintf(stderr, "GenSingleReport!\n");
  //db_->Real_GenReport();
}

bool LeveldbDB::GetLevelDBProperty(const char *key, std::string *value) {
  fprintf(stderr, "2\n");
  return db_->GetProperty(key, value);
}
