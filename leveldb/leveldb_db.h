...
class LeveldbDB : public DB {
 public:
  ...
  void GenReport_() {
    // fprintf(stderr, "leveldb gen report!\n");
    return (this->*(method_report_))();
  }

  bool GetProperty(const char *key, std::string* value) {
    fprintf(stderr, "1\n");
    return (this->*(method_property_))(key, value);
  }
 ...
 private:
  ...
  void GenSingleReport();

  bool GetLevelDBProperty(const char *key, std::string *value);
  ...
  void (LeveldbDB::*method_report_)();

  bool (LeveldbDB::*method_property_)(const char *, std::string *);
  int fieldcount_;
