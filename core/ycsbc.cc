...
void PrintStats(const char *key, const std::vector<ycsbc::DB *>& dbs) {
    std::string stats;

    for(size_t i = 0; i < dbs.size(); i++) {
        if (!dbs[i]->GetProperty(key, &stats)) {
        stats = "(failed)";
      }
    }
...
int main(const int argc, const char *argv[]) {
  ...
  if (do_load) {
    ...
    for (int i = 0; i < num_threads; ++i) {
      ...
      //fprintf(stderr, "%s", "generate mem report!\n");
    }
